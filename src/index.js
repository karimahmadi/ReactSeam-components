...
const TestSection = () => {
  ...
  useEffect(() => {
    document.addEventListener(EVENT_ERROR, e => {
      console.log('e:', e.detail);
    });

    document.addEventListener(EVENT_GENERAL_ERROR, e => {
      console.log('e:', e.detail);
    });
  }, []);

  const handleShowModalClick = () => {
    openError('این خطا است', () => {}, {
      eventType: EVENT_GENERAL_ERROR,
      data: { statueCode: 200, message: 'error' },
    });
  };
